...
namespace Harmony {
    ...
    // 1. Build Sinks based on compile-time defaults
    std::vector<spdlog::sink_ptr> sinks;

    // Console Sink
    if constexpr (DEFAULT_SINK_CONSOLE_ENABLED) {
        auto consoleSink = std::make_shared<spdlog::sinks::stdout_color_sink_mt>();
        if constexpr (!DEFAULT_SINK_CONSOLE_COLOR) {
            consoleSink->set_color_mode(spdlog::color_mode::never);
        }
        sinks.push_back(consoleSink);
    }
    ...
    // Basic File Sink
    if constexpr (DEFAULT_SINK_BASIC_ENABLED) {
        sinks.push_back(std::make_shared<spdlog::sinks::basic_file_sink_mt>(
            DEFAULT_SINK_BASIC_PATH,
            DEFAULT_SINK_BASIC_TRUNCATE
        ));
    }

    // Rotating File Sink
    if constexpr (DEFAULT_SINK_ROTATING_ENABLED) {
        size_t maxSizeBytes = static_cast<size_t>(DEFAULT_SINK_ROTATING_MAX_MB) * 1024 * 1024;
        sinks.push_back(std::make_shared<spdlog::sinks::rotating_file_sink_mt>(
            DEFAULT_SINK_ROTATING_PATH,
            maxSizeBytes,
            DEFAULT_SINK_ROTATING_MAX_FILES
        ));
    }

    // Daily File Sink
    if constexpr (DEFAULT_SINK_DAILY_ENABLED) {
        sinks.push_back(std::make_shared<spdlog::sinks::daily_file_sink_mt>(
            DEFAULT_SINK_DAILY_PATH,
            DEFAULT_SINK_DAILY_HOUR,
            DEFAULT_SINK_DAILY_MINUTE
        ));
    }

    // 2. Construct the Logger (Async vs Sync)
    if constexpr (DEFAULT_ASYNC_ENABLED) {
        spdlog::init_thread_pool(DEFAULT_ASYNC_QUEUE_SIZE, DEFAULT_ASYNC_THREAD_COUNT);
        pimpl->spd_logger = std::make_shared<spdlog::async_logger>(
            name, sinks.begin(), sinks.end(), spdlog::thread_pool(), spdlog::async_overflow_policy::block
        );
    } else {
    ...
    pimpl->spd_logger->set_pattern(DEFAULT_PATTERN);
